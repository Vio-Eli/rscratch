        ...
        Add { vec } => {
            ...
            vec.iter().for_each(|child| {
                ...
                simplify_hash3(child, hmap, id).iter().for_each(|grandchild| {
                    ...
                    let grandchild_add_hash = freeze(&Pow { base: grandchild.ptr.clone().into(), raised: Constant(grandchild.pow).into() });
                    ...
                    if let Some(hashvals) = hmap.get_mut(&HashKeys { uuid, hash: grandchild_add_hash}) {
                        ...
                        if let Some(gc_mul_ptr) = hashvals.mul_ptr {
                            unsafe {
                                *(gc_mul_ptr as *mut f64) += grandchild.mul;
                            }
                        } else {
                            // Create a new Mul block and getting the pointer to the value inside it
                            let mut new_mul_const = Constant(grandchild.mul + 1.0); // We add 1.0 because the previous value was 1.0
                            if let Constant(ref mut val) = new_mul_const {
                                hashvals.mul_ptr = Some(val);
                            }

                            // Splicing function tree and inserting new mul block
                            let new_gc = Arc::new(Mul { vec: vec![hashvals.ptr.clone().into(), Arc::new(new_mul_const)] });
                        ...
                        let (mul_ptr, pow_ptr) = match (&grandchild.mul, &grandchild.pow) {
                            (1.0, 1.0) => (None, None),
                            (mul, 1.0) => (Some(mul as *const f64), None),
                            (1.0, pow) => (None, Some(pow as *const f64)),
                            (mul, pow) => (Some(mul as *const f64), Some(pow as *const f64))
                        };
